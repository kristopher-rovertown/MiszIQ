...
// MARK: - ShapeStyle extension for theme colors
extension ShapeStyle where Self == Color {
    static var royalBlue: Color { Color.royalBlue }
...
struct AppTheme {
    ...
    static func bracketColor(for percentile: Int) -> Color {
        if percentile >= 90 {
            return Color.turquoise  // Top performers
        } else if percentile >= 70 {
            return Color.royalBlue  // High performers
        } else if percentile >= 50 {
            return Color(red: 64/255, green: 164/255, blue: 216/255)  // Mid-range (blend)
        } else {
            return Color.royalBlue.opacity(0.7)  // Developing
        }
    }
}
